	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(csrf -> csrf.disable())
				...
				.authorizeHttpRequests(
						auth -> auth.requestMatchers("/api/user/login", "/api/user/register", "/api/job/fetch/all").permitAll()
						
						// this APIs are only accessible by ADMIN
						.requestMatchers("/api/user/admin/profile/permission","/api/user/admin/fetch/all/registration","/api/user/admin/register",
								"/api/job/category/add", "/api/job/category/update",
